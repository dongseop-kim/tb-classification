...
from trainer.transforms.spatial.random_ratio import RandomRatio
from trainer.transforms.spatial.random_resize_crop import RandomResizeCrop
...
def random_resize_crop_v1(height=768, width=768, scale=0.1, border_mode=cv2.BORDER_CONSTANT, p=0.5):
...
def random_translate_v1(translate_x, translate_y, p=0.5):
    trs = Affine(translate_percent={'x': (-translate_x/100, translate_x/100),
...
def random_translate_v2(translate_x, translate_y, p=0.5):
    trs1 = Affine(translate_percent={'x': (-translate_x/100, translate_x/100),
                                     'y': 0.0})
    trs2 = Affine(translate_percent={'x': 0.0,
                                     'y': (-translate_y/100, translate_y/100)})
...
def random_rotate_v1(rotate, border_mode=cv2.BORDER_CONSTANT, p=0.5):
    ...
    if border_mode == cv2.BORDER_CONSTANT:
        params.update({'border_mode': border_mode,
                       'value': 0,
                       'mask_value': 0})
...
def random_shear_v1(x: int, y: int, border_mode=cv2.BORDER_CONSTANT, p=0.5):
    ...
    if border_mode == cv2.BORDER_CONSTANT:
        params.update({'mode': border_mode,
                       'cval': 0,
                       'cval_mask': 0})
...
def random_spatial_augment_v1(height=768,
                              width=768,
                              scale=0.1,
                              rotate=45,
                              shear_x=20,
                              ...
                              translate_y=15,
                              p=0.5):
    return A.OneOf([random_resize_crop_v1(height, width, scale, cv2.BORDER_CONSTANT),
...
def random_spatial_augment_v2(n=2,
                              height=768,
                              width=768,
                              scale=0.1,
                              rotate=45,
                              ...
                              translate_y=15,
                              p=0.5):
    return A.SomeOf([random_resize_crop_v1(height, width, scale, cv2.BORDER_CONSTANT),
                     random_rotate_v2(rotate),
                     random_shear_v2(shear_x, shear_y),
                     random_translate_v1(translate_x, translate_y)],
                    n=n,
